...
<div class="row d-flex justify-content-center">
  <div class="col-lg-8 ">
    ...
    <form action="{{ route('notfis.update',$notfi->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <select class="form-control select2" style="width: 100%;" name="client", id="client">
              ...
              <option value="{{$cl->id}}" {{ $notfi->client_id == $cl->id ? 'selected' : old('client') }} >{{ $cl->clientName }}</option>
              @endforeach
            </select>
          </div>
          ...
          <div class="form-group">
            ...
            <select class="form-control select2" style="width: 100%;" name="type", id="type">
              <option></option>
              @foreach ($type as $cl)
              <option value="{{$cl}}" {{ $notfi->type == $cl ? 'selected' : old('type') }} >{{ $cl }}</option>
              @endforeach
            </select>
          </div>
          @endrole
          <div class="form-group">
            ...
            <select class="form-control select2" style="width: 100%;" name="user", id="user">
              <option></option>
              @foreach ($users as $cl)
              <option value="{{$cl->id}}" {{ $notfi->user_id == $cl->id ? 'selected' : old('user') }} >{{ $cl->username }}</option>
              @endforeach
            </select>
          </div>
          @endcan
          <div class="form-group">
            <label for="sender">Select Sender Id</label>
            <select class="form-control select2" style="width: 100%;" name="sender", id="sender">
              <option></option>
              @foreach ($senders as $cl)
              <option value="{{$cl->name}}" {{ $notfi->sender == $cl->name ? 'selected' : old('sender') }} >{{ $cl->name }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
            ...
            <textarea class="form-control" cols=50 rows =10 name="message" id="textareaid" placeholder="Enter Message ...">
            {{ $notfi->message ? : old('message') }}
            </textarea>
          </div>
        </div>
        <!-- /.card-body -->
